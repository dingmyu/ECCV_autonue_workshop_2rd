    ...
    if args.net_type == 0:
        optimizer = torch.optim.SGD(
            [{'params': model.layer0.parameters()},
             ...
             {'params': model.ratio.parameters(), 'lr': args.base_lr * 10},
...
def train(train_loader, model, criterion, optimizer, epoch, zoom_factor, batch_size, aux_weight):
    ...
    for i, (input, target, target_ratio) in enumerate(train_loader):
        ...
        output, aux, output_ratio = model(input_var)
        ...
        target_ratio = target_ratio.float().cuda(async=True)
        target_ratio_var = torch.autograd.Variable(target_ratio)
        ratio_loss = criterion_mse(output_ratio.squeeze(3).squeeze(2), target_ratio_var) / world_size
        main_loss = criterion(output, target_var) / world_size
        aux_loss = criterion(aux, target_var) / world_size
        loss = main_loss + aux_weight * aux_loss + ratio_loss
        ...
        reduced_ratio_loss = ratio_loss.data.clone()
        reduced_aux_loss = aux_loss.data.clone()
        dist.all_reduce(reduced_loss)
        dist.all_reduce(reduced_main_loss)
        dist.all_reduce(reduced_ratio_loss)
        ...
        ratio_loss_meter.update(reduced_ratio_loss[0], input.size(0))
        ...
        if rank == 0:
            if (i + 1) % args.print_freq == 0:
                logger.info('Epoch: [{}/{}][{}/{}] '
                            ...
                            'RatioLoss {ratio_loss_meter.val:.4f} '
                            ...
                            'Accuracy {accuracy:.4f}.'.format(epoch, args.epochs, i + 1, len(train_loader),
                                                              ...
                                                              ratio_loss_meter=ratio_loss_meter,
